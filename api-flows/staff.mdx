...
The client sends an HTTP request that filters for `Received` orders from the current day, along with table number (if applicable), status, priority, and dishes.
...
In this step, the expeditor can change the priority from the default if needed (5 --\> lowest priority, 1 --\> highest priority).

### Request

The client sends an HTTP request to update an order if needed.

```
curl -X PUT "{serverURL}orders/{order id}"
```

The priority is changed to `1`:
...
The server responds with a `200 OK` status code and the response body contains the updated priority.
...
The client sends an HTTP request to update the order status from `Received` to `In Progress`.
...
_Note: I could not figure out how to get the station filter to work\! Getting a "400 bad request" & unknown query parameter "filter"._
...
The HTTP request includes the dish ID and asks to return the ingredient names.
...
The status is changed to `Ready for Pickup`.
